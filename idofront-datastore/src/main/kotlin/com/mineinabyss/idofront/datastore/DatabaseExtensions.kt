...
fun Idofront.sqliteDatabase(plugin: Plugin, path: Path, init: WriteTransaction.() -> Unit = {}): Database {
    val dbFile = plugin.dataPath / path
    // sqlite will not create missing directories, and nothing guarantees the plugin's data folder exists yet
    dbFile.parent?.createDirectories()
    val dbPath = dbFile.absolutePathString()
